...
noa_2 =
  ...
  filter(month == c(1, 7)) %>%
  summarise(avg_tmax = mean(as.numeric(tmax))) %>% 
  ggplot(aes(x = year, y = avg_tmax, color = id))+
  geom_point()+
  theme(legend.position = "none")+
  facet_grid(. ~ month)
```
Not a ton of observable differences here, besides the obvious one that avg max temp in july is hotter than in january. A couple of oddball outliers here and there, but no noticeable trends over time.
